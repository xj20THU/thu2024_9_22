...
speed_path = os.path.join(current_path,"..\\") # D:\data\THU2024_9_22\thu2024_9_22\Speed-Estimation-of-Vehicles-with-Plate-Detection-master
dir = os.path.join(speed_path,"..\\") 
dir= os.path.abspath(dir)   # D:\data\THU2024_9_22\thu2024_9_22
P1_DATA_PATH = dir + '\\P1_data'  # D:\data\THU2024_9_22\thu2024_9_22\P1_data
mp4_data = os.path.join(dir,"..\\高速公路交通流数据")
mp4_data= os.path.abspath(mp4_data)    # D:\data\THU2024_9_22\高速公路交通流数据
...
mainFilename = P1_DATA_PATH + '\\result.xlsx'
...
carCascade = cv2.CascadeClassifier(speed_path + '\\myhaar.xml')
# video = cv2.VideoCapture('C:\\Users\\SHREYA\\Desktop\\shreya\\Internship\\Videos\\a_1.avi')
video = cv2.VideoCapture(mp4_data + '\\点1\\20240501_20240501114103_20240501135755_114103.mp4')
...
def detection(image_1):
    ...
    #print("Loading model")
        ...
        filename = speed_path + '\\finalized_model.sav'
...
def trackMultipleObjects():
    ...
    out = cv2.VideoWriter(P1_DATA_PATH + '\\outpy.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (WIDTH,HEIGHT))
    ...
    while True:
        ...
        if (now.hour == 0 & now.minute == 0 & now.second == 0):
            a = 1
            counter = counter + 1
            filename = P1_DATA_PATH + '\\result2.xlsx'
		...
		# print 'new previous location: ' + str(carLocation1[i])
                if [x1, y1, w1, h1] != [x2, y2, w2, h2]:
                    ...
                    if (speed[i] != None and (x2 > x1+5 or x2 < x1-5) and (y2 > y1+5 or y2 < y1-5)): #so that even if the driver opens his seat, the code doesn't detect speed in that
                        ...
                        filename = P1_DATA_PATH + '\\mainFileVehicleID.xlsx'
                        mwb = load_workbook(filename)
                        sheet[counter] = mwb.active
